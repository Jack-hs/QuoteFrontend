...
import { DatePicker, Select, Button, Table, Card, Row, Col, InputNumber, Input, Switch, message, Modal} from 'antd';
import { CalculatorOutlined, FileExcelOutlined, DeleteOutlined, PlusOutlined,DollarOutlined } from '@ant-design/icons';
...
function App() {
  ...
  const fetchSchoolDetails = async (schoolName) => {
    ...
    try {
      const response = await axios.get(`/api/quote/school-sheets/${schoolName}`);
    ...
    try {
      ...
      const response = await axios.get(`/api/quote/school-details?schoolName=${schoolName}`);
...
const otherFeeColumns = [
  ...
  const calculate = async () => {
    ...
    try {
      ...
      const res = await axios.post('/api/quote/calculate', { ...request, Weeks: weeks });
  ...
  const exportExcel = async () => {
    try {
      const res = await axios.post('/api/quote/export', quote, { 
        responseType: 'blob',
  ...
  useEffect(() => {
    const loadSchools = async () => {
      try {
        const res = await axios.get('/api/quote/school-list');
        setSchools(res.data);
  ...
  useEffect(() => {
    ...
    const currentTotalNTD = currentTotalUSD * request.ExchangeRate;
    ...
    const currentLocalTotalPeso = quote?.localFees?.reduce((sum, item) => {
      ...
      const pesoToNT = 0.6;
      const currentLocalTotalNTD = currentLocalTotalPeso * pesoToNT;
...
const currentOtherTotalNTD = quote?.otherFees?.reduce((sum, item) => {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
      <h1 style={{ textAlign: 'center' }}>📊 報價系統</h1>
      ...
      {quote && (
        <>
            ...
            <Row gutter={16} style={{ marginTop: 16, textAlign: 'right' }}>
              <Col span={24}>
                ...
                <p style={{ color: '#888', margin: '4px 0' }}>
                  美金：台幣＝1：{request.ExchangeRate} (報價當日匯率)
                </p>
          ...
          <Card title="🇵🇭 當地雜費項目" style={{ marginTop: 24 }}>
            <Button 
              type="primary" 
